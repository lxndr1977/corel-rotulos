<div>
    ...
    <div class="no-print bg-white rounded-lg shadow mb-5">
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-4 gap-x-4 justify-between items-end">
                <div class="grid gap-y-2 ">
                    <label for="variation" class="inline-flex items-center gap-x-3 text-sm">Peso</label>
                    <div class="fi-input-wrp w-full flex rounded-lg shadow-sm ring-1 transition duration-75 bg-white dark:bg-white/5 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-2 ring-gray-950/10 dark:ring-white/20 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-600 dark:[&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-500 fi-fo-select">
                        <select wire:model="product_variation" wire:change="getVariation" name="variaton" class="fi-select-input block w-full border-none bg-transparent py-1.5 pe-8 text-base text-gray-950 transition duration-75 focus:ring-0 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] dark:text-white dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] sm:text-sm sm:leading-6 [&_optgroup]:bg-white [&_optgroup]:dark:bg-gray-900 [&_option]:bg-white [&_option]:dark:bg-gray-900 ps-3">
                            <option value="">Selecione um peso</option>
                            @foreach ($product->variations as $variation)
                                <option value="{{ $variation->id }}">{{ $variation->formattedWeight() }} {{ $variation->unitMeasurement->unit_symbol }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid gap-y-2 ">
                    ...
                    <div class="fi-input-wrp w-full flex rounded-lg shadow-sm ring-1 transition duration-75 bg-white dark:bg-white/5 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-2 ring-gray-950/10 dark:ring-white/20 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-600 dark:[&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-500 fi-fo-select">
                        <select wire:model="batch" wire:change="getBatch" name="batch" class="fi-select-input block w-full border-none bg-transparent py-1.5 pe-8 text-base text-gray-950 transition duration-75 focus:ring-0 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] dark:text-white dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] sm:text-sm sm:leading-6 [&_optgroup]:bg-white [&_optgroup]:dark:bg-gray-900 [&_option]:bg-white [&_option]:dark:bg-gray-900 ps-3">
                ...
                <div class="grid gap-y-2">
                    ...
                    <input 
                        class="fi-input block w-full bg-white dark:bg-white/5 border-gray-200 rounded-lg shadow-sm py-1.5 text-base text-gray-950 transition duration-75 placeholder:text-gray-400 focus:ring-0 focus:border-primary-600 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.400)] dark:text-white dark:placeholder:text-gray-500 dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] dark:disabled:placeholder:[-webkit-text-fill-color:theme(colors.gray.500)] sm:text-sm sm:leading-6 ps-3 pe-3" 
                        type="number" 
                        id="total_labels"
                        name="total_labels" 
                        wire:model.blur="total_labels" 
                        wire:model.change="total_labels" 
                        min="1" />
                </div>
                ...
                <div class="grid gap-y-2">
                    ...
                    <div class="fi-input-wrp flex rounded-lg shadow-sm ring-1 transition duration-75 bg-white dark:bg-white/5 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-2 ring-gray-950/10 dark:ring-white/20 [&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-600 dark:[&:not(:has(.fi-ac-action:focus))]:focus-within:ring-primary-500 fi-fo-select">
                        <select id="start_at_position" name="start_at_position" wire:model="start_at_position" 
                        wire:change="handleTotalLabelsBlur"
                         class="fi-select-input block w-full border-none bg-transparent py-1.5 pe-8 text-base text-gray-950 transition duration-75 focus:ring-0 disabled:text-gray-500 disabled:[-webkit-text-fill-color:theme(colors.gray.500)] dark:text-white dark:disabled:text-gray-400 dark:disabled:[-webkit-text-fill-color:theme(colors.gray.400)] sm:text-sm sm:leading-6 [&_optgroup]:bg-white [&_optgroup]:dark:bg-gray-900 [&_option]:bg-white [&_option]:dark:bg-gray-900 ps-3">
                            @if ($selectedVariation && $selectedVariation->labelGroup && $selectedVariation->labelGroup->labels_per_page)
                                @for ($i = 1; $i <= $selectedVariation->labelGroup->labels_per_page; $i++)
                                    <option value="{{ $i }}">Posição {{ $i }}</option>
                                @endfor
                            @else
                                <option value="" disabled>Sem posições disponíveis</option>
                            @endif
                        </select>
                    </div>
                </div>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-3 gap-x-8 items-start p-6">
                    @if ($selectedVariation 
                            && $selectedVariation->labelGroup 
                            && $selectedVariation->labelGroup->image 
                            && $selectedBatch)
                        <div class="col-span-2">
                            <p class="mb-2 font-bold">{{ $selectedVariation->labelGroup->name }}</p>
                            <img src="{{ Storage::url($selectedVariation->labelGroup->image) }}" alt="Imagem do Grupo de Rótulos" class="max-h-96">
                        </div>
                        <div>
                            <p class="mb-2 font-bold">{{ $selectedVariation->package->description }}</p>
                            <img src="{{ Storage::url($selectedVariation->package->image) }}" alt="Imagem da Embalagem" class="package-image">
                        </div>
                    @else
    ...
    @for ($current_label = 1; $current_label <= $total_labels + $start_at_position - 1; $current_label++)
        ...
        <div class="print-label-page {{ $current_label + $labels_per_page - 1 < $total_labels + $start_at_position - 1 ? 'page-break' : '' }}">
        @endif
